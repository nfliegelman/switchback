...
2. Otherwise continue ROADMAP.md at M8, the scan history logger. M0-M7 landed (v1.1.0-v1.8.0). Every availability fetch should start appending to SQLite so the demand clock starts; the crowd term and the Permit Difficulty Index both feed on it. Exception rule: pull M9 (watch mode plus Telegram) forward if the owner wants Glacier cancellation alerts now; ELF, COS, ELH are the camps to watch.
