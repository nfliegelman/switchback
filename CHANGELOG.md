...
Versioning per the 2026-07-20 owner directive: only meaningful, user-testable milestones bump the version; routine and documentation-only work ships without one. Earlier entries follow the old milestone-per-minor scheme; v2.0.0 is the full Switchback engine.

## v3.6.1 (2026-07-21)

Direction honesty, caught by you against the real AllTrails profile before calibrating, which is exactly the right instinct. The hike from Sunrise to Mystic Camp is a net-downhill day, but the sheet showed only its total climb, so it read like a grind uphill. Every day line everywhere now shows both numbers and calls the character: "9.8 mi, +1,983 ft up / -2,854 ft down, rolling, real climbing both directions", so a mostly-downhill day can never masquerade as a climb again. The underlying figures were checked against your AllTrails profile and hold up; only the telling was wrong. Also per your ask, time estimates are gone from the calibration sheet completely (they stay in the app, listed last), and the sheet's climb and descent figures now come from a single source so one row can never show two slightly different totals.

## v3.6.0 (2026-07-20)

The calibration release, shaped by your first real test drive and your Lena trip report. The launcher bug that made the app refuse to connect on your machine is fixed for good: Switchback.bat now runs the server in a visible window and waits until it actually answers before opening your browser, so a failure shows its reason instead of vanishing. Ranking now works the way you asked: days at or easier than your comfortable day count as good days, and only harder-than-comfortable is penalized, so the app stops hunting trips that merely match your limits. Any trip that sleeps every night at one camp is now called a Basecamp everywhere, whatever shape the walk in and out traces, and every layover day names its best packs-off day hikes with round-trip miles, climb, and a time at a no-pack pace, since those day hikes are the whole point of basing. The calibration sheet was rebuilt to be readable: plain stays like "2 nights at Mystic Camp campsite" with the trailhead named, every day above your comfortable numbers flagged loudly, and a note explaining that a flagged trip ranking high means its camps scored exceptionally. Terrain is now described the way you think about it: miles at each grade band first, then the separate climbs so relentless up-down-up terrain shows as, say, five distinct climbs with their sizes and grades instead of one smeared total, then total feet up and feet down, with the time estimate listed last on purpose. The sustained-steep warning now fires at twenty percent grade, matching how the Upper Lena climb actually felt. Rainier leads the calibration run as the vertical-slice park, and your two future asks are written into the plan: AllTrails beauty ratings as a camp-quality signal, and trail surface quality from map data plus review color as a second difficulty input.

## v3.5.0 (2026-07-20)

The course correction, and the first complete trip. Following your master directive, Switchback now plans whole trips instead of listing solver output. The map app's new green Plan trips button asks what a normal person deciding on a trip actually knows: when you can go (exact dates or a flexible window), how many nights, who is coming, how far and how much climbing feels comfortable versus your absolute limit (with Relaxed, Moderate, and Challenging presets you can edit), what kind of trip, whether first-come nights are acceptable, and whether you want a campground near the trailhead the night before or after. It reads your choices back in one plain sentence, then returns complete trips: every single night accounted for, from an arrival night at a real campground (Cougar Rock, White River, Ohanapecosh, curated by hand for Rainier) through each backcountry camp to an optional recovery night. Each recommendation says what is bookable right now with a link, what is first-come with no guarantee, and what Switchback honestly cannot verify; nothing is ever called available without data behind it, and when the availability service cannot be reached the app now says the empty result may be a network failure rather than pretending everything is sold out. Cards carry plain-language reasons and tradeoffs instead of decimal scores; opening one shows the night-by-night plan, the day-by-day walking, warnings, and the route on the map, with GPX download. When nothing fits, it tells you the smallest change that would work, with numbers: raise your daily max from 8 to 9.3 miles, start two days later, allow a basecamp. Twelve automated golden scenarios guard all of this offline, including the rule that no night can ever silently disappear from a plan. Also fixed: a hidden one-line error in the map app that would have made the whole page come up blank on your first real run; it had never been caught because no real browser had opened this file yet. The engine, solver, and all existing searches are untouched. The project paperwork now matches the directive too: new master-direction, current-phase, parked-features, and coverage-status documents, a roadmap rewritten into short, medium, and long horizons, and an end to bumping the version for every little change. Same-day repair, after an independent audit and still within this version: the new form had a bug that crashed it the moment you pressed Find trips, now fixed and guarded by a test that drives the real page in a real browser; the checker that guarantees no night silently disappears now measures against the dates the trip promises rather than the records it happens to have; campground facts were corrected (Ohanapecosh is closed for all of 2026 for construction and will never be offered for a 2026 night; White River is first-come, no reservations); and Rainier's status is honestly marked as blocked on your first real walkthrough rather than finished.
...
The great markdown diet, owner-prompted: sixteen root files become ten. The addendum folds into PRODUCT.md as Part 1 (and keeps its authority over Part 2, the brief); SCOPE folds into SPEC as its build-boundaries part; FUTURE moves to docs/archive; EXPERIMENTS moves to docs; and the three generated coverage files (PARKS, COLORADO, WASHINGTON) collapse into one COVERAGE.md, tiers first then both state atlases, with both CLI commands routed through a single composer so neither can clobber the other's half. Every living pointer updated; nothing erased, only merged, moved, or generated differently. The ten that remain: README, PRODUCT, ROADMAP, SPEC, COVERAGE, HANDOFF, BACKLOG, CHANGELOG, OWNER, CLAUDE.
