...
- Beyond: rubber band route builder (tap, drag, real trail animates, green in bounds, amber out; two phases per BACKLOG); dispersed itineraries experiment (auto camp nodes from named lakes and basins, synthetic availability, pilot one Weminuche drainage); GPX patch channel (owner traces win over everything); dem_trail_v1 (elevation along real polylines); Archetype B trailhead-quota parks (Yosemite, SEKI); RMNP corridor buildout; scoring calibration when the owner can react.
