...
## dem_trail_v1 results (2026-07-14, v2.23)

Benchmark: the Longmire to Klapatche chain grades 6,544 ft against the 6,450 GPS log, PLUS ONE PERCENT, from dem_line's plus seven and endpoint's minus forty-three. 32 of 33 Rainier edges regraded on real polylines (one edge has no geometry), 9 Glacier edges regraded. Teton Crest, Indian Peaks, and the Elks keep pass arithmetic untouched per the gain tier. The Elk Range suspect resolved decisively: the Maroon Zone to North Fork trail polyline measures 11.53 mi against the stored 7.19, so the display was right and the curated figure was 60 percent short; miles replaced per the v2.7 trunk-pass policy with the curated value preserved as miles_curated. ORIENTATION LESSON, now load-bearing in dem.py: geometry polylines live in canonical sorted-key orientation, so any consumer computing direction-dependent quantities MUST orient to its own a to b first; the unoriented first run graded every Northern Loop descent as a climb. Pilot residual: weminuchepilot edges do not carry inline geometry, so pilot gains remain the v2.19 line-sampled figures; carrying geometry through build_pilot is queued.

## Corridor pilot shipped (2026-07-14, v2.24): the Colorado Trail

The design held. Centerline from super-relation 3445384 (341 ways, 44,933 vertices, two-level member recurse since the CT nests segment relations), shapely buffer at the 1.5 km default in a local equirectangular scale, 23 tiles of 0.3 degrees, per-tile short-leash fetching with disk-cache resumability, per-tile transient failures skip and retry instead of aborting (one 504 taught that mid-build), dedupe by way id, clip to 3 km of centerline. Result: 2,968 ways, 2,687 trail miles, 991 KB, just under the file cap. Corridor files carry a corridor key (relation, buffer_km) for provenance.
