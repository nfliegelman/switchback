...
| Web UI hardening: Leaflet CDN fallback, interaction and accessibility fixes | P0 | AUDIT 2026-07-21 (project/audits/): vendor Leaflet locally plus an if-window.L guard plus service-worker precache so the app survives an unpkg outage (today the whole UI dies if unpkg is unreachable); plus confirm-on-quit, surface the swallowed availability-refresh error, replace the raw GPX alert, add an h1 plus visible focus plus a non-color map-status channel, darken the sub-AA muted gray. All primary-flow UI, so land together behind the standing real-browser gate; deferred whole because this container has no chromium and cannot reach unpkg |
