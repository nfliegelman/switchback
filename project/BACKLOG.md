...
| Calibration reactions fold-in | P1 | PARTIAL 2026-07-22 (v3.6.3): Rainier and RMNP effort reactions folded into scoring weights (0.4/0.6/0.15 to 0.5/0.45/0.1); comfortable day 9 mi / 2200 ft validated by the reactions and kept, then SUPERSEDED 2026-07-22 by v3.7.0: the owner said the whole ladder read a notch too hard, and the comfortable day was recalibrated DOWN to 6 mi / 1500 ft with an Extreme tier added, verified by re-scoring his ten Rainier sheet trips. 6 / 1500 is the current figure; profile.json and the plan form defaults both carry it. REMAINING: re-fold Maroon Bells after the Maroon Zone anchor fix (its day totals are corrupted today); the steep-descent term has its own row; optional further dials if Noah wants brutal-but-great-camp trips pushed below comfortable ones, a steeper above-comfortable day_fit penalty or a hardest-day weighting instead of the day mean |
| Route repair and permit-free fallbacks | P1 | corridors and dispersed are the inventory. NARROWED 2026-07-25: the manual half shipped in v3.8.0 (switchback/edit.py swaps a camp, adds or drops a layover, reverses direction, and re-validates), so what remains is the automatic half: when a trip breaks, propose the specific fix rather than making the user hunt for it. edit.py already computes every legal alternative per night, so a repair suggester is a ranking pass over edit_options output, not new machinery |
...
| Web UI interaction and accessibility hardening | P1 | AUDIT 2026-07-21 (project/audits/): confirm-on-quit, surface the swallowed availability-refresh error, replace the raw GPX alert, add an h1 plus visible focus plus a non-color map-status channel, darken the sub-AA muted gray. Primary-flow UI, so land behind the standing real-browser gate. UNBLOCKED 2026-07-25: the chromium-capable session is no longer hypothetical, chromium ships at /opt/pw-browsers in the cloud container and `pip install playwright` is all tests/test_browser.py needs to run for real (it drove the v3.8.0 edit panel end to end). The v3.8.0 pass also took the null-element crash off this list by routing the edit panel's async writes through a guarded helper; the audit's own five items are all still open |
...
| Browser test in CI | P2 | QUALITY 2026-07-21, and the last thing standing between the browser gate and being automatic (2026-07-25: proven runnable locally, still self-skips on GitHub). Install playwright plus chromium in a CI job so tests/test_browser.py actually runs on GitHub, making CI the standing real-browser gate for primary-flow UI changes (today it self-skips without chromium). Would let the tabled web UI work be verified remotely without a local browser |
