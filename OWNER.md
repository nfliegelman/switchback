...
3. Map board check: open the board (your home screen icon, or nfliegelman.github.io/switchback/board/). Bottom-left should say v3.6.4; the app caches itself, so give it a close-and-reopen or two. Tap a trip and confirm trail lines draw.
...
**NEW one-time setting (2026-07-21): turn on the safety net for main.** There is now an automatic check that runs on every change (it runs your tests and the no-dash rule). Protect the main copy so nothing can merge while a check is failing. Easiest from a computer browser: open the repo on github.com, then Settings > Branches > Add branch ruleset (or a classic branch protection rule) for the branch named `main`, and turn on both "Require a pull request before merging" and "Require status checks to pass before merging", then pick the check named `quality`. That check name only appears in the list after the first pull request has run it, so if you do not see `quality` yet, let one pull request finish and come back. Save. One time only; after this, a red check blocks a merge until it is fixed.
...
Everything now announces itself. The CLI prints "switchback v3.6.4" at the top of every command; the app window title bar shows "Switchback v3.6.4"; the map board shows it bottom-left; the .bat files print it first. If any surface does not say v3.6.4, you are running an old copy: grab the newest zip from the Releases page.
...
6. Fresh copy first: repo page > Releases > top entry > download "Source code (zip)" and extract it. Every surface should say v3.6.4; if one does not, you grabbed an old zip.
7. RE-RUN, the calibration half hour on the fixed sheet: you already did this once and it worked exactly as intended, you caught two things that were making the terrain read wrong. Both are fixed in v3.6.4. Steep downhill now shows as its own line ("0.7 mi steep downhill (20-30%)") instead of being lumped in with steep uphill, and every trip now prints its days in order (1, 2, 3) instead of jumping around. So please double-click CALIBRATE.bat again on a fresh v3.6.4 copy, wait 2 or 3 minutes, and re-read the trips now that the steepness is honest and the days are in order, typing a few words after every REACTION line (would you actually hike it? too hard, too easy, wrong camps, anything look wrong?), then save and send the file text to Claude. This corrected read is what scoring gets tuned from; your first read is not folded in, on purpose, because you gave it while steep downhill was mislabeled and it would have taught the app the wrong lesson. Two things you said are noted and NOT changed here: you judged only effort, so the camp beauty and rating side is left alone, and the picture-of-the-grades view you asked for is on the app list for later, the sheet stays as text for now. One caution unchanged: if a Maroon Bells route's miles or steepness look off, note it and move on; that corridor has a known data bug already on the fix list, so its numbers are the least trustworthy on the sheet.
